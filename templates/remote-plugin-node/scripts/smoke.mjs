...
check(toolkit.backend === 'node', 'toolkit manifest backend is node')
...
check(config.backend === 'node', 'remote config backend is node')
...
check(existsSync(resolve(root, 'backend/server.mjs')), 'backend server exists')
check(existsSync(resolve(root, 'backend/publisher.py')), 'publisher sidecar exists')
